...
def evaluate_metrics(args):
    ...
    llm, _ = llmtune.load_llm(args.model, args.weights)
    model = fix_model(llm, tokenizer, use_resize=False)
    model.config.num_beams = 5
    model.config.max_length = 50

    model = llmtune.load_adapter(model, adapter_path=args.adapter)
    ...
    def evaluate_sample(model,tokenizer,sample):
        ...
        outputs = model.generate(input_ids=input_ids, do_sample=True, top_p=0.9, max_new_tokens = 65)
    ...
    for sample in tqdm(dataset['test']['dialogue'][0:args.test_count]): ## currently only support sequential sampling (from 0 to 200 for example)
        p = evaluate_sample(model,tokenizer,sample)
